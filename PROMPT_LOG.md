...
- ショートカットキーを個別に記録・再生を無効化する機能を追加
- キャプチャ映像を仮想カメラに配信する機能を追加
- キャプチャ映像を独立したプレビューウィンドウに表示する機能を追加（画面共有用）
- 録画再生タブで複数動画の再生機能の対応
- mkv形式録画への対応

### 修正変更
- 録画中の枠線の色を調整
- 録画中のプレビューに演出表示とプレビュー自体の無効化に対応
- 対象外ウィンドウへの入力操作を記録対象から除外
- 入力されうるショートカットキー組み合わせ一覧のリストをTSV形式で作成して、標準設定として埋め込み。列はキー、有効無効フラグ、説明（マウス複合も含む）
- ショートカットキーの一覧を表示して、修飾キーなどで絞り込み、有効無効をまとめて切り替えられるような設定画面を作成。アプリの設定として保存する。
- 録画でTSVに履歴を残すとき、一覧で無効にした入力キーは記録しない
...
- 録画ウィンドウのX、Y、W、HをUpDownコントロールにして1px単位で変更可能にして、更にマウスホイールでも増減可能にして
- 座標Xの左に 「同期」チェックボックスを追加して、ONのときはウィンドウの移動や変形に連動して値が変わる。OFFのときは変わらなくて適用ボタンを押して初めて反映される
- 適用ボタンは同期がONの時は無効で、同期がOFFのときだけ有効で押せる状態にする
- 同期チェックの左に「最大化」チェックも追加して、実際のウィンドウの状態に常に連動してほしい。（同期チェックボックスの状態の影響は受けない）
- 最大化がONのときは、同期チェックボックスも、XYWHもすべて無効化して変更できなくする。ただし、同期チェックボックスには関係なく、XYWHのに表示する値は実際と同期させる
- 最大化チェックボックスはトグルボタンにして。
- 最大化トグルをOFFにしたとき、単純に最大化を解除するのではなく、最大化中のXYWHを保持してサイズを再現して。
- 同期がONでXYWHを上下や文字入力したとき瞬時に反映されるようにして
- 同期がOFFのときは、コントロールのフォーカスが移ると座標や大きさ変更が反映されることがないようにして

### 録画映像の独立プレビューウィンドウの追加（画面共有用）
- 録画ウィンドウ本体には「プレビューウィンドウを独立させる」トグルボタンを追加してONのときだけプレビューウィンドウを分離する（このとき別のウィンドウでプレビュー中ですと明記する）。OFFのときは今まで通り録画ウィンドウ内に表示する。
- 録画のプレビューを、タイトルバー、罫線など、すべてのフレームレスの独立したウィンドウに1:1解像度で表示する事ができる機能を追加して
- 独立プレビューウィンドウがOBSのウィンドウキャプチャのウィンドウ一覧に表示されるようにして。内部的なタイトル名として「ChulipVideo - 録画プレビューツール」を設定してください。
- プレビューウィンドウが右に16ピクセル余分に大きくなって黒い部分が飛び出しているので消して
- 左クリックのドラッグでウィンドウを動かしたときにチラつきが発生しているので直して
- プレビューがONのとき仮想カメラへの配信が止まっているので直して

### 録画再生タブで複数動画の再生機能の対応
- 動画を並べて配置同時再生する機能を追加して
- 録画ウィンドウの再生画面で、リストで選択した複数の動画を整列して配置し、同時に再生できるように改造して
- 配置パターンは、2（左右）、4（上下左右）、6（2段3列）、9（3段3列）、12（3段4列）、16（4段4列）のどれかで、選択アイテム数を切り上げて該当するレイアウトで並び順にプレビュー
- それぞれの動画の上にはファイル名を表示。動画の下に個別の再生ボタンとシークバーと時間を表示
- 従来から下の方にある再生・停止ボタン、シークバー、再生時間ラベルは、今後、グローバルシークバー、グローバル再生停止ボタン、グローバル再生時間と呼ぶ
- グローバルシークバー＆グローバル再生時間は、すべての動画の中で最も長い動画の時間を元に管理する。
- グローバル「再生・停止」ボタンを押したとき、開いているすべての動画を同時に再生・停止する。
- グローバル再生により、再生位置が最後まで到達してしまった個々の動画は、動画の周りをグレーにして終わったことを示す。最も長い動画が続く間はグローバル再生を止めない。
- グローバルシークバーを動かしたときは、すべての動画の再生位置をグローバルシークバーの位置に同時に変更する
- アイテムを1つしか選ばなかった場合に従来通りに表示して
- 個々の再生ボタンを押したら当該動画が再生されるようにして
- 録画ツールのウィンドウサイズが変更されたときは、真っ先に動画キャンバスを縮小させてください。普段はフレーム内いっぱいになるように引き伸ばしてください。それ以外のコントロールの高さは変わらないような設定にしてください。


### 録画ウィンドウ微修正
- 録画タブの録画対象フレームの中に、検索、対象、最大化、同期、XYWH、適用などのコントロールを移動して
- 録画対象としてデスクトップを選んでいるとき、最大化ボタンは無効化して
- 録画対象としてデスクトップを選んでいるとき、アクティブなウィンドウが当該デスクトップの中にない状態での入力操作（マウス、キー）は記録から除外して。なお、クリックは座標が当該領域外の場合にも除外して。ただし離したことの検知は領域外でも行って。
- この除外処理はプレビュー映像に対しても反映して、判定関数を共通化して
- メインウィンドウのCtrl+Cなどのショートカットキーフックを、別のウィンドウ（録画ツールなど）で行ったときに検知しないように修正して
- 録画フォーマットをmp4とmkvから選べるようにして。デフォルトはmp4。
- 「保存フォルダを開く」ボタンを追加して（削除ボタンと操作ログを開くボタンの間）
- ファイルリストに表示する拡張子をmp4とmkvに増やして
- カウントダウン秒数、最長録画時間を選べるようにして
- 一括名前変更と一括削除ができなくなっているので直して
- FPSを高くするほど再生速度が遅くなるので原因を調査して修正して（プレビュー、メインウィンドウ、詳細設定ウィンドウどれも見直して）

### サブツールの分離
- ウィンドウサイズの微調整に特化した独立したツール ChnapTool を作って
- 録画に特化した独立したツールを CapchunScreen へリネームして
- 実行時パラメータで起動したいツールを切り替えられるようにして

### ソースコードフォルダ整理
- batをbuild_toolsに移動して
- ソースコードをsrcに移動して
...
### ホイールのサポート
- マウスホイール操作も記録に取って、字幕表示するようにして（Ctrl+ホイールダウン等で使われることがある）

- マウスホイールを押したときのイベントも検知して入力履歴に残して
- 修飾キーとます
...
### クリック地点の記録と演出強化
- クリックした地点を一定時間円で表示して近くに番号を表示する機能
  - カウントリセットは、いずれかのクリックした地点周辺の映像が変化したとき。
- イベントを起こしたUIの記録機能を追加

## 操作履歴表示
- メインウィンドウで【操作履歴】ボタンを右上の「？」の左に設置して、押したらリストボックスが表示され、操作履歴が中に表示されるようにして
- 再生中は現在の地点におけるリストのアイテムをフォーカス
- ダブルクリックしたら再生箇所をその操作をした地点にジャンプ
- 記録されたテキストからキーを消す機能

- フレームの変化を割合指定できるようにする
- 選択した複数動画に対して同じクロップ処理を適用する
