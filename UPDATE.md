...
- **設定保存のグローバル化**: PNG圧縮率、フレーム除外設定、再生設定（速度、ループ、往復等）をアプリ共通設定に移行。動画を切り替えても設定が維持されるように改善
...
- **録画ツールの操作性向上**: 削除やフォルダ選択時にメインウィンドウが前面に出てしまう問題を修正。ダイアログ操作後も録画ツールが前面を維持するように改善
